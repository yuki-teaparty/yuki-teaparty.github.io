---
title: "家用扩散模型 (2.2)：Latent Consistency Models"
...
如果是[之前blog](/blog/posts/diffusion-1p5-ode.html)的读者，应该还记得Diffusion Model作为SDE，和ODE的对应关系：每一个SDE都有一个唯一对应的ODE，而一个ODE对应一个SDE族。

[之前讲EDM\[5\]的时候提到过](/blog/posts/diffusion-1p7-samplers.html)，为了让SDE的转移概率是Gaussian，通常来说SDE两项对x都必须是线性的，所以一共也没有几种可行设置。去年最流行的经典Diffusion设置自然是EDM（事实上DDIM也用了同款取值），查EDM论文表1可知其各项的取值是
...
然而，即使2022年最好的Solver也需要10+步迭代来得到一个好结果。
...
> 值得指出的一点是，虽然之前的所有blog里都使用了0，但SDE/ODE这个连续时间的formulation是不能把时刻t倒到0的（见Song之前的几篇文章），所以其实只能停在一个略大于0的时刻$\epsilon=0.002$，Denoiser各项的系数也需要相应的修正以让目标是$\mathbf{x}_\epsilon$而非$\mathbf{x}_0$。但感觉大家都把0和$\epsilon$随便混着用，明白意思就好（
...
乍一看，第一反应是哇这个形式这么好，那如果DSM Loss收敛到0了，我是不是提供任意一个image加上已知强度的噪音$\mathbf{y}+\mathbf{n}$，喂给D就能single step denoise出$\mathbf{y}$呢？
...
> 很不严谨的说，感觉这近似于要求f对t的导数处处尽可能小，收敛的理想情况下应该f对t变成常数，也就是“consistency”。
...
除此之外，另一个变化是这里$\mathbf{x}_{t_{n}}$项用的并非$\theta$（online network），而是$\theta$的EMA，记作$\theta^-$（target network），以使得结果更稳定。

> 熟悉的小伙伴可能一看到EMA和stopgrad立刻想到了contrastive learning——事实上作者说这套语言确实来自著名的BYOL\[4\]。  
>   
> 题外话：然后今天才发现Kaiming的SimSiam\[6\]指出EMA并不是BYOL不collapse的根本原因（虽然他们证明了EMA确实能涨点），而stopgrad和predictor才是BYOL不collapse的原因（
...
[之前](/blog/posts/diffusion-2-ldm.html)介绍过LDM。LCM的思路和LDM一模一样，只不过把DSM Loss换成了CD Loss（称为Latent Consistency Distillation，LCD，Algorithm 1）。
...
> 注意CFG(Classifier-free guidance)是加在noise prediction上的：  
