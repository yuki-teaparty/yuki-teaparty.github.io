...
为了提醒这是个矩阵，这次用一个大写字母 $\bm{D}(x, t) = \frac{1}{2}\bm{G}(x, t)\bm{G}^T(x, t)$ 来表示扩散系数。
...
$$
\frac{1}{p(x, t)}\nabla \cdot(p(x, t)\bm{D}^i) = \nabla\cdot \bm{D}^i + \frac{1}{p(x, t)}\bm{D}^i\cdot \nabla p(x, t) = \nabla\cdot \bm{D}^i +\bm{D}^i\cdot \nabla\log p(x, t)
$$

这里和 $\bm{G}(x, t) = g(t)$ 没有关系，对普通的G也是成立的。
...
$$
\begin{aligned} \partial_t p(x, t) &= -\nabla \cdot (\bm{f}(x, t) p(x, t)) + \nabla^2: (\bm{D}(x, t) p(x, t)) \\ &= -\nabla \cdot (\bm{f}(x, t) p(x, t))  + \sum_i \frac{\partial}{\partial x_i} (\nabla\cdot (\bm{D}^ip(x, t))) \\ &= -\nabla \cdot (\bm{f}(x, t) p(x, t))  + \sum_i \frac{\partial}{\partial x_i} (p(x, t) (\nabla\cdot \bm{D}^i + \bm{D}^i\nabla \log p(x, t))) \\ &= -\nabla \cdot (\bm{f}(x, t) p(x, t))  + \nabla\cdot (p(x, t) (\nabla\cdot \bm{D} + \bm{D}\nabla \log p(x, t))) \\ &= -\nabla \cdot ((\bm{f}(x, t) - \nabla\cdot \bm{D} - \bm{D}\nabla \log p(x, t)) p(x, t))  + \nabla^2:\bm{0} \end{aligned}
$$
...
$$
d\mathbf{x} = \{\bm{f}(\mathbf{x}, t) - \frac{1}{2} \bm{G}(t)\bm{G}^T(t)\nabla_{\mathbf{x}} \log p(\mathbf{x}, t)\}dt := \tilde{\bm{f}} (\mathbf{x}, t) dt
$$
...
具体的，对 $d\mathbf{x} = \tilde{\bm{f}} (\mathbf{x}, t)dt$ ，由instantaneous change of variables formula（见[Neural ODE](https://arxiv.org/pdf/1806.07366.pdf)的Appendix A）有

$$
\log p_0(\mathbf{x}_0) = \log p_T (\mathbf{x}_T) + \int_0^T \nabla\cdot \tilde{\bm{f}} (\mathbf{x}, t) dt
$$

其中， $\nabla\cdot \tilde{\bm{f}} (\mathbf{x}, t)$ 的计算非常昂贵，我们采用Skilling-Hutchinson trace estimator，这是一个无偏估计：

$$
\nabla\cdot \tilde{\bm{f}} (\mathbf{x}, t) =E_{\bm{\epsilon}\sim p(\bm{\epsilon})}[\bm{\epsilon}^T(\nabla\tilde{\bm{f}}) (\mathbf{x}, t) \bm{\epsilon}]
$$

这里 $\nabla\tilde{\bm{f}}$ 是jacobian， $\bm{\epsilon}$ 服从均值0方差I。

因为这是无偏估计，所以可以通过采样若干次$\bm{\epsilon}$取平均（这个e^TJe可以用gradient简单的计算）来逼近。
...
这样解出来的x的概率分布（显然是conditioned on $\theta$ 的）记作 $p_{\bm{\theta}}^{SDE}$ 。
...
当 $\bm{s}_{\bm{\theta}} \equiv \nabla_{\bm{x}} \log q_t(\bm{x}), q_T\sim \pi$ 时等号成立，且 $p_{\bm{\theta}}^{SDE}=p_{\bm{\theta}}^{ODE}=q$ 。
...
-   $p_{0 t}\left(\mathbf{x}^{\prime} \mid \mathbf{x}\right)$ 是tractable的——如果扩散模型中取 $\bm{f}(\mathbf{x}, t)$ 对x线性，那么这个概率就是一个Gaussian。
