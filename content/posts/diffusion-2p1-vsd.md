---
title: "扩散模型囫囵吞枣 (2.1)：Variational Score Distillation"
date: "2023-11-26 00:45"
slug: diffusion-2p1-vsd
order: 2.1
original_url: "https://zhuanlan.zhihu.com/p/668704605"
summary: "VSCode真好用，感觉比知乎自带的网页编辑器强大多了。之后有空的时候也会重构一下之前的几篇专栏。 这文章其实九月份就写好了，不过因为鸽子太懒所以拖到了感恩节... 动机假设我们训（下）好（载）了一个pretraine…"
source: 知乎专栏
---
VSCode真好用，感觉比知乎自带的网页编辑器强大多了。之后有空的时候也会重构一下之前的几篇专栏。

这文章其实九月份就写好了，不过因为鸽子太懒所以拖到了感恩节...

## 动机

假设我们训（下）好（载）了一个pretrained diffuser $\bm{\epsilon}_\psi(\bm{x}_t, t, c)$（也可以认为是score function，之前已经介绍过了这两者是一样的），能帮我们轻松的计算$p(\bm{x}_0|c)$。

> 其实一点也不轻松，即使是之前介绍的[DPM Solver 2m++](https://zhida.zhihu.com/search?content_id=236731458&content_type=Article&match_order=1&q=DPM+Solver+2m%2B%2B&zhida_source=entity)，也要10~20 steps（
...
$$
\bm{x}_t=\alpha_t\bm{x}_0+\sigma_t\epsilon， \bm{x}_0=g(z;\theta, c)
$$

然后最小化DSM Loss。这意味着极值处的$\nabla_\theta$是0:

$$
0=\nabla_\theta\mathcal{L}_{\text{DSM}}=\mathbb{E}_{t, \epsilon,z} \left[\omega(t)(\bm{\epsilon}_\psi(\bm{x}_t, t, c)-\epsilon)(\nabla_\theta\bm{\epsilon}_\psi(\bm{x}_t, t, c))\right]
$$

根据链式法则，

$$
\nabla_\theta\bm{\epsilon}_\psi(\bm{x}_t, t, c)=\nabla_{\bm{x}}\bm{\epsilon}_\psi(\bm{x}_t, t, c)\cdot \alpha_t\frac{\partial g(z, \theta, c)}{\partial\theta}
$$

-   后半段是我们自己设计控制的小模型，我们可以用backprop轻松算出梯度。
-   但前半段是在一个极其巨大的pretrained diffuser上求Jacobian，且不论这玩意计算起来有多昂贵，光是数值稳定性就是一大难题（显然是ill conditioned）

所以直接最小化DSM Loss非常困难。那怎么办呢？我们需要Score Distillation。

## Variational Score Distillation \[2,3\]

我们用一个新Diffuser，$\bm{\epsilon}_\phi(\bm{x}_t, t, c)$，来预测g生成样本的概率q的分数$-\sigma_t\nabla_{\bm{x}}\log q_t(\bm{x}_t, t, c)$。

我们的优化目标是(推导过程见下文)

$$
\nabla_\theta\mathcal{L}_{\text{VSD}}=\mathbb{E}_{t,z,\epsilon} \left[\omega(t)(\bm{\epsilon}_{\psi}(\bm{x}_t, t, c)-\bm{\epsilon}_\phi(\bm{x}_t, t, c))\frac{\partial g(z, \theta, c)}{\partial\theta} \right]
$$

这里$\bm{\epsilon}_{\psi}$来自预训练的大模型。

这个函数就是可以计算的了。实现上我们交替优化$\phi$和$\theta$：

-   优化$\phi$的时候使用DSM Loss（也就是$\phi$对$\bm{\epsilon}$——但计算这个DSM Loss的时候因为x是从g采样来的，实际上还是用了generator），从而让$\bm{\epsilon}_\phi(\bm{x}_t, t, c)$逼近我们想要的真值$-\sigma_t\nabla_{\bm{x}}\log q_t(\bm{x}_t, t, c)$。
...
$$
\begin{aligned}<br>0=&\nabla_\theta KL(q_\theta(x)\|p(x)) \\<br>=& \nabla_\theta \mathbb{E}_{x\sim q_\theta(x)}[\log q_\theta(x)-\log p(x)]\\<br>=& \nabla_\theta \mathbb{E}_{z\sim q_(z)}[\log q_\theta(g_\theta(z))-\log p(g_\theta(z))] \\<br>=&\mathbb{E}_{z\sim q_(z)}[ \nabla_\theta \log q_\theta(g_\theta(z))- \nabla_\theta \log p(g_\theta(z))] \\<br>=&\mathbb{E}_{z\sim q_(z)}[ \nabla_\theta \log q_\theta(x)|_{x=g_\theta(z)}+\frac{\partial x}{\partial \theta}\nabla_x\log q_\theta(x)|_{x=g_\theta(z)}- \frac{\partial x}{\partial \theta}\nabla_x \log p(x)|_{x=g_\theta(z)}] \\<br>\end{aligned}
$$

而

$$
\begin{aligned} &\mathbb{E}_{z\sim q_(z)}[ \nabla_\theta \log q_\theta(x)|_{x=g_\theta(z)}]\\ =&\mathbb{E}_{x\sim q_\theta(x)}[ \nabla_\theta \log q_\theta(x)]=\int q_\theta(x) \frac{\nabla_\theta q_\theta(x)}{q_\theta(x)} dx \\ =&\nabla_\theta \int q_\theta(x)dx\\ =&\nabla_\theta \bm{1}=0 \end{aligned}
$$

所以

$$
\begin{aligned} \nabla_\theta KL(q_\theta\|p(x)) = \mathbb{E}_{z\sim q_(z)}[(\nabla_x\log q_\theta(x)- \nabla_x \log p(x))\frac{\partial x}{\partial \theta}|_{x=g_\theta(z)}] \\ \end{aligned}
$$

到这里已经长出了VSD的主要形状（注意$\nabla_x \log p(x)$和$\bm{\epsilon}$差了个负号），剩下来的权重部分就只和t有关了。
...
$$
\nabla_\theta\mathcal{L}_{\text{SDS}}=\mathbb{E}_{t,z,\epsilon} \left[\omega(t)(\bm{\epsilon}_{\psi}(\bm{x}_t, t, c)-\bm{\epsilon})\frac{\partial g(z, \theta, c)}{\partial\theta} \right]
$$

这里$\bm{\epsilon}_{\psi}$来自预训练的大模型。

考虑VSD这个问题。给定条件c和先验z，事实上我们可以有很多个$\theta$，使得$g(z;\theta, c)$符合要求。因此我们需要的其实是一个variational的$\theta$分布——但SDS其实是$\theta \sim \delta(\theta-\theta^*)$，此时有$q_\theta \sim \mathcal{N}(\bm{x}_t|\alpha_t g(z; \theta^*, c), \sigma_t^2 \bm{I})$，于是$-\sigma_t \nabla_x\log q_\theta(x) = \bm{\epsilon}$。
...
>   
> 但话又说回来，实际上大家都负担不起train一个新diffuser，所以还是SDS用的多...本文还是以乐子为主（指鸽子发了这篇是为了空出草稿箱赶紧写下一篇）

## VSD和[GAN](https://zhida.zhihu.com/search?content_id=236731458&content_type=Article&match_order=1&q=GAN&zhida_source=entity)的关系
...
$$
\begin{aligned}<br>0 &= \nabla_\theta \mathcal{L}_G^{KL} \\<br>&= \mathbb{E}_{z\sim q_(z)} [\nabla_\theta \log \frac{1-h_\phi(x)}{h_\phi(x)}]\\<br>&\approx \mathbb{E}_{z\sim q_(z)} [\nabla_\theta \log q_\theta(g_\theta(z)) - \nabla_\theta \log p(g_\theta(z))] \\<br>&= \mathbb{E}_{z\sim q_(z)}[(\nabla_x\log q_\theta(x) - \nabla_x \log p(x))\frac{\partial x}{\partial \theta}|_{x=g_\theta(z)}] <br>\end{aligned}
$$

这样就长得和VSD（的不加noise, t==0的版本）异曲同工（甚至可以说是一模一样）了。

> 所以其实鸽子有点好奇，如果用$\mathcal{L}_G^{KL}$代替$\mathcal{L}_G$，train GAN会变得更稳定吗？  
>   
> 可惜生产中大家人均WGAN-GP，早就没有人用传统GAN了...

## 下期预告

打算看看最近很火的Latency Consistency Model。

Diffusion这领域有一个好，就是如果一个算法真管用，立马就会在知乎看见推送教程...建议以后按LoRA下载量给新算法打分嗷（x
